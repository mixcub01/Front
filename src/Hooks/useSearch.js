    ...
    const delayDebounceFn = setTimeout(async () => {
      try {
        ...
        const postPromise = axios.get(
          `https://back-yzvd.onrender.com/api/search/suggestions?q=${query}`, // ⬅️ API (1) หา Post
          { headers: { Authorization: `Bearer ${token}` } }
        );
        const userPromise = axios.get(
          `https://back-yzvd.onrender.com/api/users/search/suggestions?q=${query}`, // ⬅️ API (2) หา User
          { headers: { Authorization: `Bearer ${token}` } }
        );
